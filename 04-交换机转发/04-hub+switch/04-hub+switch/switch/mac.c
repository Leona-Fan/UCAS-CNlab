...
iface_info_t *lookup_port(u8 mac[ETH_ALEN])
{
	...
	mac_port_entry_t *max_port_entry_pos = NULL;
	pthread_mutex_lock(&mac_port_map.lock);
	list_for_each_entry(max_port_entry_pos, &max_port_map.hash_table[hash_value], list){
		if(memcpy(mac_port_entry_pos->mac,mac, ETH_ALEN) == 0){
			pthread_mutex_unlock(&mac_port_map.lock);
			return mac_port_entry_pos->iface;
		}
	}
	pthread_mutex_unlock(&mac_pot_map.lock);
	return NULL;
}
