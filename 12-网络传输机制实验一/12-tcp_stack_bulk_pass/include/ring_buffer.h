...
#include <stdlib.h>
#include <string.h>
#include <pthread.h> // to add a lock

struct ring_buffer {
	pthread_mutex_t lock; // add a lock
	int size;
...
static inline struct ring_buffer *alloc_ring_buffer(int size)
{
	...
	rbuf->size = size + 1;
	// printf("alloc %d to rbuf\n", rbuf->size);

    if (pthread_mutex_init(&rbuf->lock, NULL) != 0) { // init the clock
        free(rbuf);
        return NULL;
    }
...
static inline void write_ring_buffer(struct ring_buffer *rbuf, char *buf, int size)
{
	...
	// while (size <= 0 || ring_buffer_free(rbuf) < size)
	int len = size;
