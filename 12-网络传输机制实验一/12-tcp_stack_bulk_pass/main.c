...
{
	// 首先根据参数判断是建立server还是client
	// 如果是server，将port作为参数进入tcp_server线程，用于监听该端口
	// 如果是client，将skaddr（ip和port）作为参数传入tcp_client线程，用于主动建立连接
	pthread_t thread;
	...
	if (strcmp(args[0], "server") == 0) {
		...
		pthread_create(&thread, NULL, tcp_server, &port); // tcp_apps.c
	}
		...
		pthread_create(&thread, NULL, tcp_client, &skaddr); // tcp_apps.c
	}
	else {
		usage_and_exit(basename);
	}
...
int main(int argc, char **argv)
{
	...
	init_tcp_stack(); // tcp_sock.c

	run_application((const char *)basename(argv[0]), argv+1, argc-1); // main.c
