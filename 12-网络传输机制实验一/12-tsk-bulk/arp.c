...
void arp_send_request(iface_info_t *iface, u32 dst_ip)
{
	...
	char *request = (char *)malloc(ETHER_HDR_SIZE + sizeof(struct ether_arp));
	
	//header
	struct ether_header *eth_header = (struct ether_header *)request;
	...
	//ARP
	struct ether_arp *eth_arp = (struct ether_arp *)(request + ETHER_HDR_SIZE);
	...
	iface_send_packet(iface, request, ETHER_HDR_SIZE + sizeof(struct ether_arp));
}
...
{
	char *reply = (char *)malloc(ETHER_HDR_SIZE + sizeof(struct ether_arp));
	
	//header
	struct ether_header *eth_header = (struct ether_header *)reply;
	memcpy(eth_header->ether_dhost, req_hdr->arp_sha, ETH_ALEN);
	memcpy(eth_header->ether_shost, iface->mac, ETH_ALEN);
	eth_header->ether_type = htons(ETH_P_ARP);
	
	//ARP
	struct ether_arp *eth_arp = (struct ether_arp *)(reply + ETHER_HDR_SIZE);
	eth_arp->arp_hrd = htons(ARPHRD_ETHER);
	eth_arp->arp_pro = htons(ETH_P_IP);
	eth_arp->arp_hln = ETH_ALEN;
	eth_arp->arp_pln = 4;
	...
	iface_send_packet(iface, reply, ETHER_HDR_SIZE + sizeof(struct ether_arp));
}
