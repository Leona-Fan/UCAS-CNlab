...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	...
	list_for_each_entry(ptr,&tcp_established_sock_table[idx],hash_list){
		if(ptr->sk_sport == sport && ptr->sk_dport == dport && ptr->sk_sip == saddr && ptr->sk_dip == daddr){
			return ptr;
		}
	}
	return NULL;
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	u32 dip = ntohl(skaddr->ip);
	u16 dport = ntohs(skaddr->port);
	rt_entry_t * entry = longest_prefix_match(dip);
	if(sport == 0 || entry == NULL){
		return -1;
	}
...
//push the tcp sock into listen_queue (fang zhao shang mian)
...
struct tcp_sock *tcp_sock_accept(struct tcp_sock *tsk)
{
	...
	struct tcp_sock * tsock = tcp_sock_accept_dequeue(tsk);
	return tsock;
