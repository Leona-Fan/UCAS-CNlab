...
void arpcache_append_packet(iface_info_t *iface, u32 ip4, char *packet, int len)
{
	...
	struct arp_req *new_list = malloc(sizeof(struct arp_req));
	new_list->iface = iface;
	new_list->ip4 = ip4;
	new_list->sent = time(NULL);
	new_list->retries = 1;
	init_list_head(&(new_list->cached_packets));
	list_add_tail(&(recv_pkt->list),&(new_list->cached_packets));
	list_add_tail(&(new_list->list),&(arpcache.req_list));
...
void arpcache_insert(u32 ip4, u8 mac[ETH_ALEN])
{
	...
	if (found_valid_entry == 0) {
		...
		int index = time(NULL) % MAX_ARP_SIZE;
